...
void fileReader(string fileName, vector<string> &arr) {
    ...
    if (!inputFile) {
    	...
    	if (!outputFile) {
	        while (true) {
	        	system("cls");
				cout << "Error creating the output file: " << fileName << "\n\n";
				system("pause");
			}
	    }
    	outputFile.close();
	...
	if (!inputFile2.is_open()) {
        while (true) {
			system("cls");
			cout << "Failed to open the input file: " << fileName << "\n\n";
			system("pause");
		}
    }
    string text, line;
    while (getline(inputFile2, line)) arr.push_back(line);
	inputFile2.close();
}
	...
	while (true) {
		...
		if (hasSave) {
			...
			cout << "\n\t\tNo save file detected."
				 << "\n\n\t\tType [Create] to create a save file. Leave blank otherwise."
				 << "\n\n\t\t > ";
			getline(cin, x);
			if (x == "Create") {
		...
		cout << "\n\n\n\n\n\n\tType [Multiplayer] to enable Player 2. Leave blank otherwise."
			 << "\n\n\t > ";
		getline(cin, x);
		if (x == "Multiplayer") {
			multiplayer = true;
		...
		while (true) {
			...
			food(rng(0, fullX - 1), rng(0, fullY - 1), map);
			while (true) {
				if (dead && dead2) break;
				decayer(map);
				if (!dead) {
